 ...
 * de ventas, detalle de compras, traslados entre almacenes y lo cargado en
 * cuentas de bahia (todas tenian onDelete('cascade') hacia producto.PRO_Id)
 * -- un solo click en "Eliminar" en el listado de productos corrompia
 * ventas y compras ya declaradas/facturadas. Se cambia a RESTRICT (el
 * default de MySQL al omitir onDelete): si el producto tiene historial, la
 * base de datos rechaza el DELETE en vez de arrastrarlo.
 * ProductoController::destroy() ya se ajusta aparte para desactivar el
 * producto en lugar de borrarlo cuando esto pasa.
 */
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('traslado_detalle', function (Blueprint $table) {
            $table->dropForeign('TRD_KFR2');
            $table->foreign('PRO_Id', 'TRD_KFR2')->references('PRO_Id')->on('producto');
        });
    ...
    public function down(): void
    {
        ...
        Schema::table('traslado_detalle', function (Blueprint $table) {
            $table->dropForeign('TRD_KFR2');
            $table->foreign('PRO_Id', 'TRD_KFR2')->references('PRO_Id')->on('producto')->onDelete('cascade');
        });
