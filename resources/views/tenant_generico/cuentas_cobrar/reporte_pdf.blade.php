...
<head>
    ...
    <style>
        ...
        .cuenta-header .venta-titulo {
            font-size: 12.5px;
            font-weight: 700;
            color: #1F2937;
        }

        .cuenta-header .cliente-info {
            font-size: 10px;
            color: #4B5563;
        }
...
<body>
    ...
    <header>
        <div class="titulo-reporte">Reporte de Cuentas por Cobrar</div>
        <div class="subtitulo-reporte">Generado el {{ $fechaGeneracion->format('d/m/Y H:i') }}</div>
    </header>

    <footer>
        Reporte de Cuentas por Cobrar &middot; Generado el {{ $fechaGeneracion->format('d/m/Y H:i') }}
    </footer>
    ...
    @forelse ($cuentas as $cuenta)
        <div class="cuenta-bloque">
            <table class="cuenta-header">
                <tr>
                    <td style="width:70%;">
                        <span class="venta-titulo">Venta #{{ $cuenta->VEN_Id }} &mdash; {{ $cuenta->CLI_Nombre }}</span><br>
                        <span class="cliente-info">Doc: {{ $cuenta->CLI_NumDocumento ?: '-' }} &nbsp;|&nbsp; Emisión: {{ $cuenta->CXC_FechaEmision }}</span>
                    </td>
                    <td style="width:30%; text-align:right; vertical-align:middle;">
                        @if ($cuenta->CXC_Estado == \App\Models\Tenant\CuentaCobrar::ESTADO_PAGADO)
            ...
            <table class="montos-cuenta">
                <tr>
                    <td>
                        ...
                        <span class="m-valor">S/ {{ number_format($cuenta->CXC_MontoTotal, 2) }}</span>
                    </td>
                    <td>
                        <span class="m-label">Adelanto</span>
                        <span class="m-valor">S/ {{ number_format($cuenta->CXC_MontoAdelanto, 2) }}</span>
                    </td>
                    <td>
                        <span class="m-label">Abonado</span>
                        <span class="m-valor">S/ {{ number_format($cuenta->CXC_MontoAbonado, 2) }}</span>
                    </td>
                    <td>
                        <span class="m-label">Pendiente</span>
                        <span class="m-valor">S/ {{ number_format($cuenta->CXC_MontoPendiente, 2) }}</span>
                    </td>
                </tr>
            </table>

            @if ((int) ($cuenta->CXC_TieneCuotas ?? 0))
                        ...
                        <tbody>
                            @foreach ($cuenta->cuotas as $cuota)
                                <tr>
                                    ...
                                    <td>{{ \Illuminate\Support\Carbon::parse($cuota->CCC_FechaVencimiento)->format('d/m/Y') }}</td>
                                    ...
                                    <td>{{ $cuota->CCC_Estado == \App\Models\Tenant\CuentaCobrarCuota::ESTADO_PAGADO ? 'Pagado' : 'Pendiente' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    @empty
        <div class="sin-resultados">No hay cuentas por cobrar que coincidan con los filtros seleccionados.</div>
    @endforelse
