...
@push('scripts')
    <script>
        ...
        function cargarDesdeCotizacion(cotizacionId) {
            $.get('{{ tenant_url('tenant.ventas.cotizacion.show', ['cotizacion' => ':id']) }}'.replace(':id', cotizacionId))
                .done(function(data) {
        ...
        function marcarCotizacionConvertida(ventaId) {
            ...
            $.ajax({
                url: '{{ tenant_url('tenant.ventas.cotizacion.marcarConvertida', ['cotizacion' => ':id']) }}'.replace(':id', cotizacionId),
                method: 'POST',
                data: {
                    venta_id: ventaId,
                    _token: $('meta[name="csrf-token"]').attr('content')
