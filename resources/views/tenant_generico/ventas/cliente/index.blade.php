        ...
        <div class="col-4">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-text"></p>
                    <form method="POST" id="cliente_form" action="{{ route('tenant.ventas.cliente.store', tenant('id')) }}">
                        @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#tabla_cliente').DataTable({
                ...
                ajax: "{{ route('tenant.ventas.cliente.index', tenant('id')) }}",
                columns: [{
                        data: 'CLI_Id',
                        name: 'CLI_Id'
                    },
            ...
            $('body').on('click', '.editCliente', function() {
                ...
                $.get('{{ route('tenant.ventas.cliente.edit', ['cliente' => ':cliente', 'tenant' => tenant('id')]) }}'.replace(':cliente',
                        Cliente_id_edit),
                    function(result) {
                        console.log(result);
            ...
            $('#updateBtn').click(function(e) {
                ...
                if (form.checkValidity()) {
                    ...
                    $.ajax({
                        data: $('#cliente_form').serialize(),
                        url: '{{ route('tenant.ventas.cliente.update', ['cliente' => ':cliente', 'tenant' => tenant('id')]) }}'
                            .replace(
            ...
            $('body').on('click', '.eyeCliente', function() {
                ...
                $.get('{{ route('tenant.ventas.cliente.show', ['cliente' => ':cliente', 'tenant' => tenant('id')]) }}'.replace(':cliente',
                        Cliente_id_ver),
                    function(data) {
            ...
            $('body').on('click', '.deleteCliente', function() {
                ...
                if ($confirm == true) {
                    $.ajax({
                        ...
                        url: '{{ route('tenant.ventas.cliente.destroy', ['cliente' => ':cliente', 'tenant' => tenant('id')]) }}'
                            .replace(
