        ...
        <div class="col-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-text"></p>
                    <form method="POST" id="metodoPago_form" action="{{ route('tenant.ventas.metodopago.store', tenant('id')) }}">
                        @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#tabla_metodoPago').DataTable({
                ...
                ajax: "{{ route('tenant.ventas.metodopago.index', tenant('id')) }}",
                columns: [{
                        data: 'MEP_Id',
                        name: 'MEP_Id'
                    },
            ...
            $('#saveBtn').click(function(e) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('tenant.ventas.metodopago.store', tenant('id')) }}",
                    type: "POST",
                    dataType: 'json',
            ...
            $('body').on('click', '.editMetodoPago', function() {
                ...
                $.get('{{ route('tenant.ventas.metodopago.edit',['metodopago' => ':metodopago', 'tenant' => tenant('id')]) }}'.replace(':metodopago', MetodoPago_id_edit),
                    function(result) {
                        console.log(result);
            ...
            $('#updateBtn').click(function(e) {
                ...
                $.ajax({
                    data: $('#metodoPago_form').serialize(),
                    url: '{{ route('tenant.ventas.metodopago.update', ['metodopago' => ':metodopago', 'tenant' => tenant('id')]) }}'.replace(
            ...
            $('body').on('click', '.deleteMetodoPago', function() {
                ...
                if ($confirm == true) {
                    $.ajax({
                        ...
                        url: '{{ route('tenant.ventas.metodopago.destroy',  ['metodopago' => ':metodopago', 'tenant' => tenant('id')]) }}'.replace(
