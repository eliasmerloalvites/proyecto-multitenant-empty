        ...
        <div class="card cot-card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3 cot-header-row">
                    ...
                    <a href="{{ tenant_url('tenant.ventas.cotizacion.create') }}" class="cot-btn-nueva">
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#tabla_cotizaciones').DataTable({
                ...
                serverSide: true,
                order: [[0, 'desc']],
                ajax: "{{ tenant_url('tenant.ventas.cotizacion.index') }}",
                columns: [
            ...
            $('body').on('click', '.eyeCotizacion', function() {
                ...
                $.get('{{ tenant_url('tenant.ventas.cotizacion.show', ['cotizacion' => ':id']) }}'.replace(':id', id),
                    function(data) {
                        ...
                        if (parseInt(data.cotizacion.COT_Estado) === 1) {
                            var urlEditar = '{{ tenant_url('tenant.ventas.cotizacion.edit', ['cotizacion' => ':id']) }}'.replace(':id', id);
            ...
            function abrirPdfCotizacion(id) {
                var url = '{{ tenant_url('tenant.ventas.cotizacion.pdf', ['cotizacion' => ':id']) }}'.replace(':id', id);
            ...
            $('body').on('click', '.anularCotizacion', function() {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: '{{ tenant_url('tenant.ventas.cotizacion.anular', ['cotizacion' => ':id']) }}'.replace(':id', id),
                        type: 'POST',
