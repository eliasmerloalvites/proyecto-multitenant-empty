...
<head>
    ...
    <title>Orden de Servicio - Mantenimiento Actividad Variada</title>
...
<body>
    ...
    <div class="container">
            ...
            <table class="header-table">
                <tr>
                    ...
                    <td width="47%">
                        ...
                        <div class="subtitle">
                            Mantenimiento Actividad Variada
                        </div>
                    </td>

                    <td width="25%">
                        <div class="header-info">
                            <strong>N° ORDEN:</strong> MAV-{{ str_pad($mttoPreventivo->MAV_Id, 6, '0', STR_PAD_LEFT) }} <br>
                            <strong>FECHA:</strong> {{ date('d/m/Y', strtotime($mttoPreventivo->MAV_FechaCreacion)) }} <br>
                            <strong>HORA:</strong> {{ date('H:i:s', strtotime($mttoPreventivo->MAV_FechaCreacion)) }} <br>
                                ...
                                <table class="client-table">
                                    <tr>
                                        <td width="50%">
                                            ...
                                            <div class="client-value">{{ $mttoPreventivo->MAV_Propietario }}</div>
                                        </td>
                                        <td width="50%">
                                            <div class="client-label">PLACA</div>
                                            <div class="client-value">{{ $mttoPreventivo->MAV_Placa }}</div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <div class="client-label">TELÉFONO</div>
                                            <div class="client-value">{{ $mttoPreventivo->MAV_celular }}</div>
                                        </td>
                                        <td>
                                            <div class="client-label">UNIDAD</div>
                                            <div class="client-value">{{ $mttoPreventivo->MAV_Unidad }}</div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <div class="client-label">KILOMETRAJE</div>
                                            <div class="client-value">{{ $mttoPreventivo->MAV_KMEntrada }}</div>
                                        </td>
                                        <td>
                                ...
                                <table class="client-table">
                                    <tr>
                                        ...
                                        <td width="50%">
                                            ...
                                            <div class="client-value">
                                                <span class="badge-estado" style="background:{{ $coloresEstado[$mttoPreventivo->MAV_Estado] ?? '#64748b' }}">
                                                    {{ $mttoPreventivo->MAV_Estado }}
                                                </span>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            ...
                                            <div class="client-value {{ empty($mttoPreventivo->MAV_FechaInicio) ? 'blank' : '' }}">
                                                {{ $mttoPreventivo->MAV_FechaInicio ? date('d/m/Y H:i', strtotime($mttoPreventivo->MAV_FechaInicio)) : 'Sin registrar' }}
                                            </div>
                                        </td>
                                        <td>
                                            <div class="client-label">FECHA DE ENTREGA</div>
                                            <div class="client-value {{ empty($mttoPreventivo->MAV_FechaTermino) ? 'blank' : '' }}">
                                                {{ $mttoPreventivo->MAV_FechaTermino ? date('d/m/Y H:i', strtotime($mttoPreventivo->MAV_FechaTermino)) : 'Sin registrar' }}
                                            </div>
                                        </td>
                                    </tr>
                                </table>
                ...
                <tr>
                    <td>
                        <div class="small-box">
                            ...
                            <div class="small-body">{{ $mttoPreventivo->MAV_DetalleIngreso }}</div>
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="small-box">
                            ...
                            <div class="small-body">{{ $mttoPreventivo->MAV_DetalleObservacion }}</div>
                        </div>
                    </td>
                </tr>
            </table>
        ...
        <div class="section no-break">
            ...
            <div class="large-body">{{ $mttoPreventivo->MAV_DetalleRealizado }}</div>
        </div>

        <!-- CORRECCION -->
        <div class="section no-break">
            <div class="section-title">CORRECCIÓN DE OBSERVACIONES</div>
            <div class="large-body">{{ $mttoPreventivo->MAV_CorrecionObservacion }}</div>
        </div>
                ...
                <tr>
                        ...
                        <table class="parts-table">
                            ...
                            <tbody>
                                @foreach($detalle as $det)
                                    <tr>
                                        <td class="text-center">{{ $det->MAVD_Item }}</td>
                                        <td>{{ $det->MAVD_Descripcion }}</td>
                                        <td class="text-center">1</td>
                                        <td class="text-right">S/ {{ number_format($det->MAV_Precio,2) }}</td>
                                    </tr>
                                @endforeach
                                @if($total_detalle > 0)
                                    <tr>
                    ...
                    <td width="32%" style="padding-left:10px">
                        <div class="service-box">
                            ...
                            <div class="service-body">{{ $mttoPreventivo->MAV_ProximoCambioAceite }}</div>
                        </div>
                        <div class="service-box">
                            <div class="service-title">PRÓXIMO SERVICIO</div>
                            <div class="service-body">{{ $mttoPreventivo->MAV_ProximoServicio }}</div>
                        </div>
                    </td>
                </tr>
            </table>
                    ...
                    @foreach ($imagenes->chunk(2) as $chunk)
                        <tr>
                            @foreach ($chunk as $img)
                                @php
                                    $path = public_path(ltrim($img->MAVI_url, '/'));
                                    $base64 = null;
                                    if (file_exists($path)) {
                                        $type = pathinfo($path, PATHINFO_EXTENSION);
                                        $data = file_get_contents($path);
        ...
        @php
            ...
            $verifItems = [
                'MAV_VerifArranque' => 'Arranque correcto',
                'MAV_VerifLuces' => 'Luces',
                'MAV_VerifDireccionales' => 'Direccionales',
                'MAV_VerifNivelAceite' => 'Nivel de aceite',
                'MAV_VerifPruebaRuta' => 'Prueba de ruta',
                'MAV_VerifLavado' => 'Lavado',
            ];
        @endphp
        ...
        <div class="section no-break">
            ...
            <div class="section-body">
                @if ($mttoPreventivo->MAV_Recomendacion)
                    @if ($mttoPreventivo->MAV_RecomendacionPrioridad)
                        <span class="badge-estado" style="background:{{ $coloresPrioridad[$mttoPreventivo->MAV_RecomendacionPrioridad] }}">
                            PRIORIDAD {{ $mttoPreventivo->MAV_RecomendacionPrioridad }}
                        </span>
                    @endif
                    <div style="margin-top:6px; font-size:11px; color:#334155;">{{ $mttoPreventivo->MAV_Recomendacion }}</div>
                @else
        ...
        <div class="section no-break">
            ...
            <div class="section-body">
                ...
                @if ($mttoPreventivo->MAV_VerifOtros)
                    <div style="margin-top:6px; font-size:10.5px;"><strong>Otros:</strong> {{ $mttoPreventivo->MAV_VerifOtros }}</div>
                @endif
                <div style="margin-top:8px; font-size:10.5px;">
                    <strong>Conforme para entrega:</strong>
                    <span class="rec-tag {{ $mttoPreventivo->MAV_VerifConforme ? 'rec-tag-ok' : 'rec-tag-blank' }}">
                        {{ $mttoPreventivo->MAV_VerifConforme ? 'SÍ' : 'PENDIENTE' }}
                    </span>
                    &nbsp;&nbsp; <strong>Técnico:</strong> {{ $mttoPreventivo->personal }}
                    &nbsp;&nbsp; <strong>Fecha:</strong>
                    {{ $mttoPreventivo->MAV_FechaTermino ? date('d/m/Y', strtotime($mttoPreventivo->MAV_FechaTermino)) : 'Sin registrar' }}
                </div>
            </div>
        </div>
