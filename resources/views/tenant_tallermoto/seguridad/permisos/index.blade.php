        ...
        <div class="col-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-text"></p>
                    <form  id="PermisoForm" name="PermisoForm" action="{{--tenant_url('tenant.seguridad.permiso.store')--}}" >
                        @csrf
...
<script>
    ...
    $(document).ready(function() {
        ...
        var table = $('#table-permisos').DataTable({
            ...
            ajax: "{{ tenant_url('tenant.seguridad.permiso.index') }}",
            columns: [{
                    data: 'id',
                    name: 'id'
                },
        ...
        $('#saveBtn').click(function(e) {
            ...
            if (form.checkValidity()) {
                $.ajax({
                    ...
                    url: "{{ tenant_url('tenant.seguridad.permiso.store') }}",
                    type: "POST",
                    dataType: 'json',
        ...
        $('body').on('click', '.deletePermiso', function() {
            ...
            if ($confirm == true) {
                $.ajax({
                    ...
                    url: '{{ tenant_url('tenant.seguridad.permiso.destroy', ['permiso' => ':permiso', tenant('id')]) }}'.replace(':permiso', Permiso_id_delete).replace(':tenant', '{{ tenant() }}'),
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(data) {
        ...
        $('body').on('click', '.editPermiso', function() {
            ...
            $.get('{{ tenant_url('tenant.seguridad.permiso.edit', ['permiso' => ':permiso', tenant('id')]) }}'.replace(':permiso', Permiso_id_edit),
                function(data) {
                    console.log(data);
        ...
        $('#updateBtn').click(function(e) {
            ...
            $.ajax({
                data: $('#PermisoForm').serialize(),
                url: '{{ tenant_url('tenant.seguridad.permiso.update', ['permiso' => ':permiso', tenant('id')]) }}'.replace(':permiso', Permiso_id_update).replace(':tenant', '{{ tenant() }}'),
                type: "PUT",
                dataType: 'json',
                success: function(data) {
                    console.log('Success:', data);
        ...
        $('body').on('click', '.eyePermiso', function() {
            ...
            $.get('{{ tenant_url('tenant.seguridad.permiso.show', ['permiso' => ':permiso', 'tenant' => ':tenant']) }}'.replace(':permiso', Permiso_id_ver).replace(':tenant', '{{ tenant() }}'),
                function(data) {
                    console.log(data);
