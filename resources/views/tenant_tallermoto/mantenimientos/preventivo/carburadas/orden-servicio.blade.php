...
<head>
    ...
    <title>Orden de Servicio - Mantenimiento Preventivo Carburada</title>
...
<body>
    ...
    <div class="container">
                ...
                <tr>
                    ...
                    <td width="25%">
                        <div class="header-info">
                            <strong>N° ORDEN:</strong> MPC-{{ str_pad($mttoPreventivo->MPC_Id, 6, '0', STR_PAD_LEFT) }} <br>
                            <strong>FECHA:</strong> {{ date('d/m/Y', strtotime($mttoPreventivo->MPC_FechaCreacion)) }} <br>
                            <strong>HORA:</strong> {{ date('H:i:s', strtotime($mttoPreventivo->MPC_FechaCreacion)) }} <br>
                                ...
                                <table class="client-table">
                                    <tr>
                                        <td width="50%">
                                            ...
                                            <div class="client-value">{{ $mttoPreventivo->MPC_Propietario }}</div>
                                        </td>
                                        <td width="50%">
                                            <div class="client-label">PLACA</div>
                                            <div class="client-value">{{ $mttoPreventivo->MPC_Placa }}</div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <div class="client-value">{{ $mttoPreventivo->MPC_Unidad }}</div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <div class="client-label">KILOMETRAJE</div>
                                            <div class="client-value">{{ $mttoPreventivo->MPC_KMEntrada }}</div>
                                        </td>
                                        <td>
                                ...
                                <table class="client-table">
                                    <tr>
                                        ...
                                        <td width="50%">
                                            ...
                                            <div class="client-value">
                                                <span class="badge-estado" style="background:{{ $coloresEstado[$mttoPreventivo->MPC_Estado] ?? '#64748b' }}">
                                                    {{ $mttoPreventivo->MPC_Estado }}
                                                </span>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            ...
                                            <div class="client-value {{ empty($mttoPreventivo->MPC_FechaInicio) ? 'blank' : '' }}">
                                                {{ $mttoPreventivo->MPC_FechaInicio ? date('d/m/Y H:i', strtotime($mttoPreventivo->MPC_FechaInicio)) : 'Sin registrar' }}
                                            </div>
                                        </td>
                                        <td>
                                            <div class="client-label">FECHA DE ENTREGA</div>
                                            <div class="client-value {{ empty($mttoPreventivo->MPC_FechaTermino) ? 'blank' : '' }}">
                                                {{ $mttoPreventivo->MPC_FechaTermino ? date('d/m/Y H:i', strtotime($mttoPreventivo->MPC_FechaTermino)) : 'Sin registrar' }}
                                            </div>
                                        </td>
                                    </tr>
                                </table>
                ...
                <tr>
                    <td>
                        <div class="small-box">
                            ...
                            <div class="small-body">{{ $mttoPreventivo->MPC_DetalleIngreso }}</div>
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="small-box">
                            ...
                            <div class="small-body">{{ $mttoPreventivo->MPC_DetalleObservacion }}</div>
                        </div>
                    </td>
                </tr>
            </table>
        ...
        <div class="section no-break">
            ...
            <div class="large-body">{{ $mttoPreventivo->MPC_DetalleRealizado }}</div>
        </div>
                        ...
                        <table class="parts-table">
                            ...
                            <tbody>
                                @foreach($detalle as $det)
                                    <tr>
                                        <td class="text-center">{{ $det->MPCD_Item }}</td>
                                        <td>{{ $det->MPCD_Descripcion }}</td>
                                        <td class="text-center">1</td>
                                        <td class="text-right">S/ {{ number_format($det->MPC_Precio,2) }}</td>
                                    </tr>
                                @endforeach
                                @if($total_detalle > 0)
                                    <tr>
                ...
                <table class="service-table">
                    ...
                    @if($mttoPreventivo->MPC_Det1 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det1 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det1 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                    ...
                    @if($mttoPreventivo->MPC_Det2 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det2 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det2 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                    ...
                    @if($mttoPreventivo->MPC_Det3 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det3 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det3 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPC_Det4 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det4 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det4 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPC_Det5 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det5 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det5 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPC_Det6 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det6 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det6 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPC_Det7 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det7 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det7 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                    ...
                    @if($mttoPreventivo->MPC_Det8 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det8 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det8 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                                <table style="width: 100%;">
                                    <tr>
                    ...
                    @if($mttoPreventivo->MPC_Det11 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPC_Det11 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPC_Det11 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                                <table style="width: 100%;">
                                    <tr>
                    ...
                    @foreach ($imagenes->chunk(2) as $chunk)
                        <tr>
                            @foreach ($chunk as $img)
                                @php
                                    $path = public_path(ltrim($img->MPCI_url, '/'));
                                    $base64 = null;
                                    if (file_exists($path)) {
                                        $type = pathinfo($path, PATHINFO_EXTENSION);
                                        $data = file_get_contents($path);
        ...
        <div class="section no-break">
            ...
            <div class="section-body">
                @if ($mttoPreventivo->MPC_Recomendacion)
                    @if ($mttoPreventivo->MPC_RecomendacionPrioridad)
                        <span class="badge-estado" style="background:{{ $coloresPrioridad[$mttoPreventivo->MPC_RecomendacionPrioridad] }}">
                            PRIORIDAD {{ $mttoPreventivo->MPC_RecomendacionPrioridad }}
                        </span>
                    @endif
                    <div style="margin-top:6px; font-size:11px; color:#334155;">{{ $mttoPreventivo->MPC_Recomendacion }}</div>
                @else
        ...
        <div class="section no-break">
            ...
            <div class="section-body">
                ...
                @if ($mttoPreventivo->MPC_VerifOtros)
                    <div style="margin-top:6px; font-size:10.5px;"><strong>Otros:</strong> {{ $mttoPreventivo->MPC_VerifOtros }}</div>
                @endif
                <div style="margin-top:8px; font-size:10.5px;">
                    <strong>Conforme para entrega:</strong>
                    <span class="rec-tag {{ $mttoPreventivo->MPC_VerifConforme ? 'rec-tag-ok' : 'rec-tag-blank' }}">
                        {{ $mttoPreventivo->MPC_VerifConforme ? 'SÍ' : 'PENDIENTE' }}
                    </span>
                    &nbsp;&nbsp; <strong>Técnico:</strong> {{ $mttoPreventivo->personal }}
                    &nbsp;&nbsp; <strong>Fecha:</strong>
                    {{ $mttoPreventivo->MPC_FechaTermino ? date('d/m/Y', strtotime($mttoPreventivo->MPC_FechaTermino)) : 'Sin registrar' }}
                </div>
            </div>
        </div>
