...
<head>
    ...
    <title>Orden de Servicio - Mantenimiento Preventivo Inyectada</title>
...
<body>
    ...
    <div class="container">
                ...
                <tr>
                    ...
                    <td width="25%">
                        <div class="header-info">
                            <strong>N° ORDEN:</strong> MPI-{{ str_pad($mttoPreventivo->MPI_Id, 6, '0', STR_PAD_LEFT) }} <br>
                            <strong>FECHA:</strong> {{ date('d/m/Y', strtotime($mttoPreventivo->MPI_FechaCreacion)) }} <br>
                            <strong>HORA:</strong> {{ date('H:i:s', strtotime($mttoPreventivo->MPI_FechaCreacion)) }} <br>
                                ...
                                <table class="client-table">
                                    <tr>
                                        <td width="50%">
                                            ...
                                            <div class="client-value">{{ $mttoPreventivo->MPI_Propietario }}</div>
                                        </td>
                                        <td width="50%">
                                            <div class="client-label">PLACA</div>
                                            <div class="client-value">{{ $mttoPreventivo->MPI_Placa }}</div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <div class="client-label">TELÉFONO</div>
                                            <div class="client-value">{{ $mttoPreventivo->MPI_celular }}</div>
                                        </td>
                                        <td>
                                            <div class="client-label">UNIDAD</div>
                                            <div class="client-value">{{ $mttoPreventivo->MPI_Unidad }}</div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <div class="client-label">KILOMETRAJE</div>
                                            <div class="client-value">{{ $mttoPreventivo->MPI_KMEntrada }}</div>
                                        </td>
                                        <td>
                                ...
                                <table class="client-table">
                                    <tr>
                                        ...
                                        <td width="50%">
                                            ...
                                            <div class="client-value">
                                                <span class="badge-estado" style="background:{{ $coloresEstado[$mttoPreventivo->MPI_Estado] ?? '#64748b' }}">
                                                    {{ $mttoPreventivo->MPI_Estado }}
                                                </span>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            ...
                                            <div class="client-value {{ empty($mttoPreventivo->MPI_FechaInicio) ? 'blank' : '' }}">
                                                {{ $mttoPreventivo->MPI_FechaInicio ? date('d/m/Y H:i', strtotime($mttoPreventivo->MPI_FechaInicio)) : 'Sin registrar' }}
                                            </div>
                                        </td>
                                        <td>
                                            <div class="client-label">FECHA DE ENTREGA</div>
                                            <div class="client-value {{ empty($mttoPreventivo->MPI_FechaTermino) ? 'blank' : '' }}">
                                                {{ $mttoPreventivo->MPI_FechaTermino ? date('d/m/Y H:i', strtotime($mttoPreventivo->MPI_FechaTermino)) : 'Sin registrar' }}
                                            </div>
                                        </td>
                                    </tr>
                                </table>
                ...
                <tr>
                    <td>
                        <div class="small-box">
                            ...
                            <div class="small-body">{{ $mttoPreventivo->MPI_DetalleIngreso }}</div>
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="small-box">
                            ...
                            <div class="small-body">{{ $mttoPreventivo->MPI_DetalleObservacion }}</div>
                        </div>
                    </td>
                </tr>
            </table>
        ...
        <div class="section no-break">
            ...
            <div class="large-body">{{ $mttoPreventivo->MPI_DetalleRealizado }}</div>
        </div>

        <!-- CORRECCION -->
        <div class="section no-break">
            <div class="section-title">CORRECCIÓN DE OBSERVACIONES</div>
            <div class="large-body">{{ $mttoPreventivo->MPI_CorrecionObservacion }}</div>
        </div>
                ...
                <tr>
                        ...
                        <table class="parts-table">
                            ...
                            <tbody>
                                @foreach($detalle as $det)
                                    <tr>
                                        <td class="text-center">{{ $det->MPID_Item }}</td>
                                        <td>{{ $det->MPID_Descripcion }}</td>
                                        <td class="text-center">1</td>
                                        <td class="text-right">S/ {{ number_format($det->MPI_Precio,2) }}</td>
                                    </tr>
                                @endforeach
                                @if($total_detalle > 0)
                                    <tr>
                    ...
                    <td width="32%" style="padding-left:10px">
                        <div class="service-box">
                            ...
                            <div class="service-body">{{ $mttoPreventivo->MPI_ProximoCambioAceite }}</div>
                        </div>
                        <div class="service-box">
                            <div class="service-title">PRÓXIMO SERVICIO</div>
                            <div class="service-body">{{ $mttoPreventivo->MPI_ProximoServicio }}</div>
                        </div>
                    </td>
                </tr>
            </table>
                ...
                <table class="service-table">
                    ...
                    @if($mttoPreventivo->MPI_Det1 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det1 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det1 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                    ...
                    @if($mttoPreventivo->MPI_Det2 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det2 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det2 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                    ...
                    @if($mttoPreventivo->MPI_Det3 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det3 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det3 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det4 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det4 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det4 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det5 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det5 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det5 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det6 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det6 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det6 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det7 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det7 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det7 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                    ...
                    @if($mttoPreventivo->MPI_Det8 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det8 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det8 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                                <table style="width: 100%;">
                                    <tr>
                    ...
                    @if($mttoPreventivo->MPI_Det9 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det9 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det9 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det10 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det10 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det10 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det11 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det11 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det11 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det12 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det12 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det12 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det13 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det13 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det13 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det14 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det14 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det14 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if(!empty($mttoPreventivo->MPI_Det15))
                        <tr>
                            ...
                            <td class="status">
                                <span class="ref-value">{{ $mttoPreventivo->MPI_Det15 }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if(!empty($mttoPreventivo->MPI_Det16))
                        <tr>
                            ...
                            <td class="status">
                                <span class="ref-value">{{ $mttoPreventivo->MPI_Det16 }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det17 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det17 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det17 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                                <table style="width: 100%;">
                                    <tr>
                    ...
                    @if($mttoPreventivo->MPI_Det18 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det18 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det18 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @if($mttoPreventivo->MPI_Det19 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det19 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det19 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference">
                                <table style="width: 100%;">
                                    <tr>
                    ...
                    @if($mttoPreventivo->MPI_Det20 == 'SI')
                        <tr>
                            ...
                            <td class="status">
                                <span class="status-{{ $mttoPreventivo->MPI_Det20 == 'SI' ? 'ok' : 'no' }}">{{ $mttoPreventivo->MPI_Det20 == 'SI' ? 'OK' : 'NO' }}</span>
                            </td>
                            <td class="reference"></td>
                        </tr>
                    @endif
                    ...
                    @foreach ($imagenes->chunk(2) as $chunk)
                        <tr>
                            @foreach ($chunk as $img)
                                @php
                                    $path = public_path(ltrim($img->MPII_url, '/'));
                                    $base64 = null;
                                    if (file_exists($path)) {
                                        $type = pathinfo($path, PATHINFO_EXTENSION);
                                        $data = file_get_contents($path);
        ...
        @php
            ...
            $verifItems = [
                'MPI_VerifArranque' => 'Arranque correcto',
                'MPI_VerifLuces' => 'Luces',
                'MPI_VerifDireccionales' => 'Direccionales',
                'MPI_VerifNivelAceite' => 'Nivel de aceite',
                'MPI_VerifPruebaRuta' => 'Prueba de ruta',
                'MPI_VerifLavado' => 'Lavado',
            ];
        @endphp
        ...
        <div class="section no-break">
            ...
            <div class="section-body">
                @if ($mttoPreventivo->MPI_Recomendacion)
                    @if ($mttoPreventivo->MPI_RecomendacionPrioridad)
                        <span class="badge-estado" style="background:{{ $coloresPrioridad[$mttoPreventivo->MPI_RecomendacionPrioridad] }}">
                            PRIORIDAD {{ $mttoPreventivo->MPI_RecomendacionPrioridad }}
                        </span>
                    @endif
                    <div style="margin-top:6px; font-size:11px; color:#334155;">{{ $mttoPreventivo->MPI_Recomendacion }}</div>
                @else
        ...
        <div class="section no-break">
            ...
            <div class="section-body">
                ...
                @if ($mttoPreventivo->MPI_VerifOtros)
                    <div style="margin-top:6px; font-size:10.5px;"><strong>Otros:</strong> {{ $mttoPreventivo->MPI_VerifOtros }}</div>
                @endif
                <div style="margin-top:8px; font-size:10.5px;">
                    <strong>Conforme para entrega:</strong>
                    <span class="rec-tag {{ $mttoPreventivo->MPI_VerifConforme ? 'rec-tag-ok' : 'rec-tag-blank' }}">
                        {{ $mttoPreventivo->MPI_VerifConforme ? 'SÍ' : 'PENDIENTE' }}
                    </span>
                    &nbsp;&nbsp; <strong>Técnico:</strong> {{ $mttoPreventivo->personal }}
                    &nbsp;&nbsp; <strong>Fecha:</strong>
                    {{ $mttoPreventivo->MPI_FechaTermino ? date('d/m/Y', strtotime($mttoPreventivo->MPI_FechaTermino)) : 'Sin registrar' }}
                </div>
            </div>
        </div>
